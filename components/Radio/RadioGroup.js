...
export default {
  ...
  computed: {
    classes() {
      return {
        'vc@radio-group': true,
        'vc@radio-group--column': this.column && !this.row,
        'vc@radio-group--row': this.row,
      }
    }
  },
  ...
  methods: {
    getRadios() {
      return this.$children
        .filter((child) => child.$el.classList.contains('vc@radio'))
    },
    ...
    radioBlur(e) {
      if (!e.relatedTarget || !e.relatedTarget.classList.contains('vc@radio')) {
        this.shouldValidate = true;
