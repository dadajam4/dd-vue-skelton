...
export default {
  ...
  watch: {
    ...
    listIndex(next, prev) {
      ...
      if (next in this.tiles) {
        this.tiles[next].classList.add('vc@list__tile--highlighted');
        this.$refs.content.scrollTop = next * 48;
      }

      prev in this.tiles &&
        this.tiles[prev].classList.remove('vc@list__tile--highlighted');
    }
  },
  ...
  methods: {
    ...
    getTiles() {
      this.tiles = this.$refs.content.querySelectorAll('.vc@list__tile');
    }
  }
}