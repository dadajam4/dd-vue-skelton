  ...
  methods: {
    genTHead() {
      ...
      if (this.$scopedSlots.headers) {
        ...
        const checkbox = this.$createElement('vt@checkbox', {
          props: {
            // dark: this.dark,
            // light: this.light,
    ...
    genHeaderData(header, children) {
      ...
      classes.push(`text--${header.align || 'left'}`);
    ...
    genHeaderSortingData(header, children, data, classes) {
      ...
      classes.push('vc@sortable');
      const icon = this.$createElement('vt@icon', {
        props: {
          sm: true,
        }
      }, 'arrow-upward');
      ...
      if (beingSorted) {
        classes.push('vc@active');
        if (pagination.descending) {
          classes.push('vc@desc');
