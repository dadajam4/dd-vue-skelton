<style lang="scss" scoped>
</style>

<template>
  <vt@page>
    <vt@page-header>Badges</vt@page-header>
    <vt@page-intro>
      ほげほげ
    </vt@page-intro>

    <vt@page-section title id="standard-display" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge left>
          <span slot="badge">6</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>

        <vt@badge right class="vc@m-l--md">
          <span slot="badge">88</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>

        <vt@badge class="vc@m-l--xl">
          <span slot="badge">816</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="colors" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge overlap left success>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge overlap left warning>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge overlap left error>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge overlap left layer-color="indigo">
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge overlap left background-color="shades-white" border-color="purple" text-color="lime">
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="outline" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge outline overlap left success>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge outline overlap left warning>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
        <vt@badge outline overlap left error>
          <span slot="badge">1</span>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="icon" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge overlap left success>
          <vt@icon slot="badge">check</vt@icon>
          <vt@icon xl>user-circle</vt@icon>
        </vt@badge>

        <vt@badge overlap right class="vc@m-l--xl" layer-color="indigo">
          <vt@icon slot="badge">bell</vt@icon>
          <vt@icon xl>users</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="inline" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge left warning>
          <span slot="badge">2</span>
          <span>Examples</span>
        </vt@badge>

        <vt@badge right class="vc@m-l--xl" error>
          <vt@icon slot="badge">bell</vt@icon>
          <span>2 Messages</span>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="visibility" data-anchor-point>
      <div class="vc@text--center vc@m-b--lg" @click="visible = !visible">
        <vt@btn secondary>toggle</vt@btn>
      </div>

      <div class="vc@text--center">
        <vt@badge left v-model="visible">
          <span slot="badge">6</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>

        <vt@badge right class="vc@m-l--md" v-model="visible">
          <span slot="badge">88</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>

        <vt@badge class="vc@m-l--xl" v-model="visible">
          <span slot="badge">816</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>

    <vt@page-section title id="clickable" data-anchor-point>
      <div class="vc@text--center">
        <vt@badge @click-badge="clickCount = clickCount + 1">
          <span slot="badge">{{clickCount}}</span>
          <vt@icon xl>shopping-cart</vt@icon>
        </vt@badge>
      </div>
    </vt@page-section>
  </vt@page>
</template>

<script>
export default {
  $_anchors,

  head() {
    return {
      title: 'Badges',
    }
  },

  data() {
    return {
      visible: true,
      clickCount: 0,
    }
  },

  computed: {
  },

  methods: {
  },

  created() {
  },
}
</script>
